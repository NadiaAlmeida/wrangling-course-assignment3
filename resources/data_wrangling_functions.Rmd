...
## fast key funktions 
```{r}
# command shift c -> put lines in #
# option - ->    <- 
# option and select lines with mouse makes you able to write in multiple lines 
```
...
# %>% uses output of a line as the input of the next line, will run slower than |>

# |> might break tho by not letting the code run 
...
# dplyr::filter() -> is for rows so left right 
...
# dplyr::select() ->is for columns so up down 
...
# dplyr::rename() -> (new name = name in file)
...
# dplyr::summarize() takes all rows and gives you one like the mean of something
...
# dplyr::count() # gives you the amount of each response, good to check all response types
# count is group_by and summarise in one
...
# dplyr::distinct() # gives you every response that there is so that you know all answer types
                    # also deletes duplicates that happen with full_joins
```
...
# dplyr::group_by() # or ".by =" does the code for every row in the chosen group like the mean in summarize for every individual instead of one single mean 
...
# dplyr::full_join() retains all data from x and y. problem: get many duplicates which is bad 

# dplyr::left_join() join matching values from y to x

# dplyr::right_join() join matching values from x to y

# dplyr::inner_join() join data and retain only rows with matches

# dplyr::semi_join() Return rows of x that have a match in y
...
```{r}
#filter(str_detect(trial_id, "2_")) will show you the rows with a trial_id containing 2_
#can also be used with mutate 

#str_detect using regex for more complex stuff, ask chatgpt
```
