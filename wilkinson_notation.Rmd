...
Benefit: isn't fully math, helps specify models 

`y ~ x1 + x2` -> B1 and B2, `y ~ x1 * x2` -> B1, B2, B interaction x(UV) causes changes in y(AV) but not the other way around 
...
beta: what is the y increase with a one point increase in x 
p value: tells you bx(beta estimate) CI excludes 0, slope(gerade) is not parallel to x axis-> tells you it's not nothing 
`y ~ 1 ` ->intercept only model -> mean 
e: standard deviation from the slope 
...
res <- lmer(formula = count_selfreport_measures ~ 1 + year + (1 | subfield/journal),#(1 | subfield/journal) is the random effects, the more RE the wider and unpräziser the 95% CI becomes but allowes more general predictions, variation depending on which journal in which subfield  
            data = dat)
