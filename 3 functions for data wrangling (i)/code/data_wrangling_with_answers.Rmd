...
data_demographics_trimmed |>
  filter(item == "age") |>
  count(response) #one non numeric response "old"

data_demographics_clean_age <- data_demographics_trimmed |>
  mutate(response = ifelse(response == "old", NA, response),
         response = as.numeric(response)) |>
  filter(item == "age")|>
  rename(age = response) #or select(unique_id, age) to only get these columns and allgender responses as NA
...
data_selfreport_cleaned <- data_selfreport_trials|>
  mutate(response = ifelse(response %in% as.character(seq(1:7)), response, NA_integer_),
         response = as.numeric(response)) # no longer shows warning from only using 1:7
...
# you could use mutate(response = na_if("Ctrl + 'B'", NA, response)) to remove the response that does not correspond to a likert response but if there are multiple unwanted answers then it takes more time than to just filter after wanted responses instead
data_selfreport_cleaned2 <- data_selfreport_trials|>
...
data_selfreport_tidy<- data_selfreport_cleaned %>% 
  summarise(mean= mean(response, na.rm = TRUE),
            ...
            max= max(response, na.rm = TRUE)) #by using filter(!is.na(response)) you can clearly see how many rows are not included -> more transparent regarding just to how many people the mean is in regards to
...
data_selfreport_scored <- data_selfreport_cleaned %>% 
  group_by(unique_id) %>% 
